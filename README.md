...
有 mission 在跑时,输入框下面常驻一行(密排,和 pi 自己的状态行同一种排版):

```
● 执行 2/6 · T3 领域模型与 NextDue 纯函数 a=2/3 · 16min $0.76 · recurring-tui-api
```

相位 · 进度 · 当前任务(取标题里第一个断点前的标题头) · 已跑时长与花费 · mission id。
终端窄了从右往左丢,相位和当前任务留到最后。判定在跑、侦查在扇出、熔断临界、
等待换脑时,才会多出第二行 —— 其余时候它就是一行。

**空输入框按 ↓,焦点落到这一行上**(整行高亮,不挂按键提示 —— 那一行是永久占位):

| 键 | |
|---|---|
| `↵` | 打开 `/mission status` |
| `↓` | 若有子 agent 在跑,继续往下进入执行看板,`↑↓` 逐条看轨迹、`↵` 看动作原文 |
| `↑` / `Esc` | 退回输入框 |

输入框里只要有字,方向键就还是编辑器的;`/missions`、评审页开着时也不接管。
...
输入框下方的常驻卡同时多出一行 `侦查扇出 1/4 · S2 读 src/repo/user.ts · 1m12s`。
