...
export const QUICK_PHASE_RULES: Record<string, string> = {
	...
	act: [
		...
		"",
		"这一轮结束后系统会自动升档到 standard,并**带着你的诊断回到 PLAN 重新规划** ——",
		"那条一句话判据会被摊开成一组冻结的验收标准 + verify.sh,判定比现在更严,不是更松。",
		"所以这段诊断不只是给下一次尝试看的,它是下一份计划的输入:说清楚「按什么标准才算真做完了」。",
		"这是设计好的出口,不是故障,你不需要做任何事来触发它。",
		"",
		"这一档**没有 `mission_escalate`**(工具集里就没有),也不需要它:升档本身就是升级,",
		"而且是往严了升。quick 唯一的判定依据是那条已冻结的判据,任何相位都不会再让你改它。",
	].join("\n"),
};
